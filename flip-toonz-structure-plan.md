...
- **Refill is once per turn, not once per action.** CONFIRMED verbatim from the printed rulebook ("Refill the Market"): *"Once a player has completed their actions, if there are fewer than five cards in the market, reveal cards from the toon deck until there are five available and rearrange the cards in the market so that they are ordered by rank (lowest to the left and highest to the right)."* A hire/dismiss mid-turn leaves its slot empty; the gap only fills when that player's Market phase ends. (An earlier draft of this bullet paraphrased this as "after any change," which reads as per-action and is wrong — see item 16's correction below and phases.ts's hire() header comment.) Note the order within that one refill — **refill, then re-sort** — which means buying a cheap low-rank card can make a card you left behind more expensive.
- **Horse is the one confirmed exception.** Its own card text/FAQ: *"If a player hires a horse, immediately discard any number of cards in the market, reveal an equal number of new cards from the deck, and arrange them by rank."* This is a card-specific IMMEDIATE refill, distinct from the once-per-turn refill above — it fires the moment Horse is hired, not deferred to end of turn.
...
- **`snake`** — **now fully encoded (2026-08-17), see §14.** Was blocked on the FAQ's nested case (a Peacock stacked via Snake needing its When-Hired ability resolved "after the Flip phase is complete") for lack of a deferred-post-Flip onHire mechanism; that mechanism now exists (`GameState.pendingOnHireCardIds`, resolved in `runPostFameHooks`). Mongoose's *own* dismissed-card-onDismiss gap is a different, more general limitation (no card's onDismiss fires outside `dismiss()`) and remains open — not closed by this fix.
